...
<tr>
<td>RUT</td><td>Estado Cuenta</td><td>Monto Deuda</td>
<?php 
$y = date('Y'); 
$m=date('n');
if ($m==12){$m=1;} else {$y--;} 
...
<tr>
<td><?php echo $key;?></td>
<td><?php echo $val['estado_cuenta'];?></td>
<td class="right"><?php echo number_format($val['monto_deuda'],0,',','.');?></td>
<?php 
$y = date('Y'); 
$m=date('n');
if ($m==12){$m=1;} else {$y--;} 
?>
<?php for ($i=1;$i<=12;$i++):?>
    <?php if (array_key_exists($y.'-'.$m,$val['pagos'])):?>
    <td class="right"><?php echo number_format($val['pagos'][$y.'-'.$m]['monto_pagado'],0,',','.'); $totales[$i]+=$val['pagos'][$y.'-'.$m]['monto_pagado'];?></td>
    <?php else:?>
    <td class="right">0</td>
    <?php endif;?>
    ...
    <?php /*MES ACTUAL*/?>
    <?php if ($i==12):?>
    <?php if (array_key_exists(date('Y').'-'.date('n'),$val['pagos'])):?>
    <td class="right"><?php echo number_format($val['pagos'][date('Y').'-'.date('n')]['monto_pagado'],0,',','.'); $totales[$i]+=$val['pagos'][date('Y').'-'.date('n')]['monto_pagado'];?></td>
    <?php else:?>
    <td class="right">0</td>
    <?php endif;?>
    <?php endif;?>
