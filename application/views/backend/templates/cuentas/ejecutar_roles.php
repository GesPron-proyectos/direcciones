...
<div class="agregar-noticia">
    ...
    <div width="20%" class="agregar">
    	<a id="consultar" class="nueva" href="<?php echo site_url();?>/admin/roles/revisar_cuentas/">CONSULTAR BASES</a>
    </div>
	<?php } ?>
...
function consultar_bases(){
	$("#mask").css({"display":"block", "height": $(document).height()});
	actionData = { "consulta": 1 }
	$.ajax({
	  type: "POST",
	  dataType: 'json',
	  data: actionData,
	  url: $('#site_url').val()+'/admin/roles/revisar_cuentas/',
	  success: function(response){
		$("#mask").css({"display":"none"});
		var d = response.result;
		if(d==1){
