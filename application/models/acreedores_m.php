<?php
class Acreedores_M extends EMP_Model{
	public function EMP_Model() { $this->__construct(); }

	public function __construct() {
		parent::__construct();
		$this->_table = 'acreedor';
		$this->primary_key = 'idacreedor';
		$this->alias = 'a';
...
/*
	...
	public function get_cuentas($idcuenta= ''){
		...
		$cols[] = 'jue.jurisdiccion AS jurisdiccionE';
		$cols[] = 'c.receptor AS receptor';
		$cols[] = 'c.id_usuario AS id_usuario';
		$cols[] = 'CONCAT(tp.tipo,"-",c.rol,"-",a.anio) AS rol';
		$cols[] = 'c.rol1 AS rol1';
		$cols[] = 'c.rol1_y AS rol1_y';
		$cols[] = 'c.rol2 AS rol2';
		$cols[] = 'c.rol2_y AS rol2_y';
		$cols[] = 'c.id_castigo AS id_castigo';
		$cols[] = 'c.id_usuario AS id_usuario';
		$cols[] = 'c.no_llamar AS no_llamar';
		$cols[] = 'c.notaria_personeria AS notaria_personeria';
		$cols[] = 'c.titular_personeria AS titular_personeria';
		$cols[] = 'c.fecha_escritura_personeria AS fecha_escritura_personeria';
		...
		$cols[] = 'c.venc_plazo_ord AS venc_plazo_ord';
		
		$cols[] = 'c.aud_det_pasivo AS aud_det_pasivo';
		$cols[] = 'c.aud_constitutiva AS aud_constitutiva';
		$cols[] = 'c.deleg_poder AS deleg_poder';
		$cols[] = 'c.agregado AS agregado';
		$cols[] = 'c.cartas_enviadas AS cartas_enviadas';

        $cols[] = 'p.fecha_suscripcion AS fecha_suscripcion';
		$cols[] = 'c.id_administrador AS id_administrador';
		$cols[] = 'c.notificaciones AS notificaciones';
		$cols[] = 'c.fecha_termino_suspension AS fecha_termino_suspension';
		...
		$cols[] = 'li.razon_social AS nombre_liquidador';
		...
		$cols[] = 'c.tipo_usuario AS tipo_usuario';
		$cols[] = 'c.exorto AS exorto';
		$cols[] = 'c.id_estado_cuenta AS id_estado_cuenta';
		$cols[] = 'c.id_procurador AS id_procurador';

//ACTUALIZAR DATOS DE LA CUENTA - SELECT 
		$cols[] = 'c.id_mandante AS id_mandante';
		$cols[] = 'c.id_liquidador AS id_liquidador';
		$cols[] = 'c.id_martillero AS id_martillero';
		$cols[] = 'c.id_institucion AS id_institucion';
		$cols[] = 'c.id_ministro_de_fe AS id_ministro_de_fe';

		
		$cols[] = 'c.fecha_crea AS fecha_crea';		
		$cols[] = 'c.id_tribunal_ex';
		$cols[] = 'c.id_distrito_ex';
		$cols[] = 'CONCAT(tpp.tipo,"-",c.rolE,"-",an.anio) AS rolEx';
		$cols[] = 'c.rolE AS rolE';
		$cols[] = 'c.anioE AS anioE';




              //$cols[] = 'me.marca AS marca';
	  	$cols[] = 'c.letraE AS letraE';
	      //$cols[] = 'tre.tribunal AS tribunale';
              //$cols[] = 'diste.tribunal AS distritoE';
		$cols[] = 'c.operacion AS operacion';

		//QUERY DATOS OBTENIDOS 
		$this->db->select($cols);
        ...
        $this->db->join("liquidador li","li.id=c.id_liquidador","left");
        ...
        $this->db->join("tipo_usuario tu","tu.id=c.tipo_usuario","left");
...
/*
	...
	*/
	...
	public function get_etapas_juicio(){
		...
		$this->db->from("acreedor c");
		//$this->db->join("0_usuarios usr","usr.id=c.id_usuario AND usr.activo='S'");
        $this->db->join("estado_acreedor seta","seta.id=c.id_etapa","left");
        //$this->db->join("s_tribunales tri","tri.id=c.id_tribunal","left");
        
        $this->db->where(array('c.activo' => 'S'));	
       // $this->db->order_by("c.fecha_asignacion", "DESC");
        $this->db->group_by('c.idacreedor');
        $query = $this->db->get();
		return  $query->result();
	}
...
*/
    ...
    public function get_cuentas_alertas_proceso($where=array(),$like=array()){
		...
		//$cols[] = 'c.id AS id';
		$cols[] = 'pa.n_pagare AS id';
		$cols[] = 'cta.id_procurador AS id_procurador';
		$cols[] = 'cta.id_mandante AS id_mandante';
		$cols[] = 'cta.id_estado_cuenta AS id_estado_cuenta';
		$cols[] = 'cta.rol AS rol';
		$cols[] = 'cta.medio_contacto_otro AS medio_contacto_otro';
		$cols[] = 'cta.medio_contacto AS medio_contacto';
		$cols[] = 'cta.medio_informado_otro AS medio_informado_otro';
		$cols[] = 'cta.medio_informado AS medio_informado';
		$cols[] = 'cta.id_castigo AS id_castigo';
		$cols[] = 'cta.rol2 AS rol2';
		$cols[] = 'cta.fecha_asignacion AS fecha_asignacion';
		$cols[] = 'cta.fecha_crea AS fecha_crea';
		$cols[] = 'DATEDIFF(NOW(),cta.fecha_asignacion) as duracion';
		//$cols[] = 'DATEDIFF(NOW(),ce.fecha_etapa) as ultimo_dia';
		$cols[] = 'DATEDIFF( NOW() , cta.fecha_etapa ) AS dias_diferencia';
		$cols[] = 'e.texto_alerta AS texto_alerta';
		$cols[] = "(DATEDIFF( NOW() , cta.fecha_etapa ) - dias_alerta) AS dias_alerta_diferencia";
		...
		$cols[] = 'ce.id_cuenta AS id_cuenta';
		$cols[] = 's_cuenta.estado AS estado';
		$cols[] = 'cta.fecha_etapa AS fecha_etapa';
		//$cols[] = 'ce.id AS id';
		$cols[] = 'trib.tribunal as tribunal_padre_comuna';
		$cols[] = 'trie.tribunal as tribunalE';
		$cols[] = 'diste.tribunal as DistritoE';
		
		$cols[] = 'cta.rolE as rolE';
	  ...
	  //////############ CONSULTA PARA EXPORTAR EXCEL CUENTAS#######///////
        ...
        */
		...
		$this->db->select(
			...
			cta.rol AS rol,
		    mand.codigo_mandante AS codigo_mandante,	
			usr.nombres AS usuarios_nombres,
			usr.ap_pat AS usuarios_ap_pat,
			usr.ap_mat AS usuarios_ap_mat, 
			usr.rut AS usuario_rut, 
			cta.fecha_asignacion AS fecha_asignacion,
			cta.exorto,			
			cta.rolE,
			a.anio AS anio,
			...
			an.anio AS anioE,
			DATEDIFF( NOW() , cta.fecha_etapa ) AS dias_diferencia,
			DATEDIFF( NOW() , cta.fecha_etapa - etap.dias_alerta) AS dias_alerta_diferencia,
			etap.dias_alerta AS dias_alerta,
			operacion,
			etap.etapa AS etapa, 
