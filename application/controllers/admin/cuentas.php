...
class Cuentas extends CI_Controller {
	...
	function __construct() {
		...
		$this->load->model ( 'consulta_pjud_m');
	...
	public function distribuir($action=''){
		...
		$where = "sistema = 'na' and tipoc != 'C' and tipo_causa = 2";
		$config3 = $this->db->select('*')->where($where)->get('configuracion')->result();
		
		$where = "sistema = 'na' and tipoc != 'C' and tipo_causa = 1";
		$config4 = $this->db->select('*')->where($where)->get('configuracion')->result();

		$configs = array_merge($config1, $config2, $config3, $config4);
		...
		foreach($configs as $key => $value){
			...
			if($value->id_procurador){
				$procurador = $this->procurador_m->get_by(array('id'=>$value->id_procurador));
				//echo '-->'.$value->id_procurador; die;
				
				$where['nombres'] = trim(strtoupper($procurador->nombre)).' '.trim(strtoupper($procurador->apellido));
			}
			if($value->tipo_causa == 1){
				if($value->sistema == 'apel'){
					...
					if($value->id_juzgado){
						if(!$value->id_distrito)
							$tribunal = 1;
						$where['trib.id'] = $value->id_juzgado;
					}
					if($value->id_comuna){
						$where['cta.id_comuna'] = $value->id_comuna;
					}

					if($value->sistema == 'na'){
						//$where['sistema'] = substr($value->sistema, 0, 3);
						if(isset($where['nombres']) && $where['nombres'] != 'MICHEL VALDIVIA'){
							unset($where['nombres']);
						}
						if($tribunal){
						...
						$query = $this->db->select('cta.id AS id
												...
												->where('seleccionado = 0') // and id_tribunal > 0
												->where($where)
												->where($where_str)
												->group_by("cta.id")
												->get();
			...
			else{ //Juicios complementarios Tipo Causa 2
				$where = array();
				//$where_str = "sistema is not null and sistema not in ('', 'cae', 'cat', 'sup', 'apel')";
				$where_str = "sistema is not null and sistema in ('laboral', 'penal', 'cobranza')";
				if($value->estado)
					$where['estado'] = $value->estado;
				if($value->id_distrito){
					$where['trib.jurisdiccion_id'] = $value->id_distrito;
					$this->db->join("tribunales_complementarios trib", "trib.id = cta.id_tribunal", "left");
				}
				if($value->id_juzgado){
					$where['trib.id'] = $value->id_juzgado;
					if(!$value->id_distrito)
			...
			if($result){
				if($value->id_designado)
					$cuentas[$value->id_designado.'-'.$value->grupo][] = $result;
				else
	...
	function enviar_correo($data){ //print_r($data); die;
                    ...
                    color: white;
                        ...
                        <body>
                            ...
                            <label style="margin-bottom:20px;">
                                Estimado(a), en el adjunto se detallan movimientos de estados diarios de causas:
                            </label>
                        </body>
                        </html>';
            ...
            foreach($value as $k => $v){
                foreach ($v as $w => $x){
                    ...
                    if(isset($x->ubicacion)){
						$arr = explode('-', $key);
                        $procurador = $this->procurador_m->get_by(array('id' => $arr[0]));
		...
		$url_img = base_url()."images/generarpdf.gif";
        ...
        foreach ($data as $key => $value){
			...
			$interno = "http://192.168.1.185:81/";
			$externo = "http://200.72.179.3:81/";
            $body1 = $body2 = $body3 = $body4 = $body5 = '';
            $cuerpo1 = $cuerpo2 = $cuerpo3 = $cuerpo4 = $cuerpo5 ='';
            $causas1 = $causas2 = $causas3 = $causas4 = $causas5 = 0;
            foreach($value as $s => $t){
                $find = $apel = $labr = $labr_na = $penal = $cobrz = false;
                foreach ($t as $x => $y){
					if(!in_array($y->id, $arr_ids)){
						$arr_ids[] = $y->id;
						if(!empty($y)){
							if(isset($y->id_na) && $y->id_na)
								$arr_ids_na[] = $y->id_na;
							...
							if(isset($y->rit) && isset($y->id_na)){
								$fecha_ingreso = '';
								if($y->fecha_ingreso!='' && $y->fecha_ingreso!='0000-00-00'){
									$fecha_ingreso = date('d-m-Y', strtotime($y->fecha_ingreso));
								}
								$cuerpo3.= "<tr><td>{$y->rit}</td><td>{$y->ruc}</td><td>{$fecha_ingreso}</td>".
										   "<td>{$y->caratulado}</td><td>{$y->tribunal}</td></tr>";
								
								$find = $labr = $labr_na = true;
								$causas3++;
							}
							elseif(isset($y->rit)){
								$fecha_ingreso = $fecha_etapa = '';
								if($y->fecha_ingreso!='' && $y->fecha_ingreso!='0000-00-00'){
									$fecha_ingreso = date('d-m-Y', strtotime($y->fecha_ingreso));
								}
								...
								$procuradorX = $y->procurador;
								$cuerpo4.="<tr><td>{$y->rut}</td><td>{$y->rit}</td><td>{$y->ruc}</td><td>{$y->caratulado}</td>".
										  "<td>{$y->tribunal}</td><td>{$fecha_etapa}</td><td>{$y->etapa_cuenta}</td>".
										  "<td>{$y->estado}</td><td>{$procuradorX}</td></tr>";
								
								$find = $labr = true;
								$causas4++;
							}
							elseif(isset($y->rol_interno)){
								$fecha_ingreso = $fecha_etapa = '';
								if($y->fecha_ingreso!='' && $y->fecha_ingreso!='0000-00-00'){
									$fecha_ingreso = date('d-m-Y', strtotime($y->fecha_ingreso));
								}
								...
								$procuradorX = $y->procurador;
								$cuerpo5.="<tr><td>{$y->rut}</td><td>{$y->rol_interno}</td><td>{$y->rol_unico}</td><td>{$y->caratulado}</td>".
										  "<td>{$y->tribunal}</td><td>{$fecha_etapa}</td><td>{$y->etapa_cuenta}</td>".
										  "<td>{$y->estado}</td><td>{$procuradorX}</td></tr>";
							...
							if(!isset($y->ubicacion) && !isset($y->rit) && !isset($y->rol_interno)){
								$fecha_etapa = '';
								if($y->fecha_etapa!='' && $y->fecha_etapa!='0000-00-00'){
									$fecha_etapa = date('d-m-Y', strtotime($y->fecha_etapa));
								}
								...
								elseif(isset($y->id_na) && $y->id_na && !$y->etapa_cuenta){
									$y->etapa_cuenta = 'Favor verificar si corresponde a su cartera, si no corresponde contactarse con mesa de operaciones';
								}
								
								if(isset($y->id_na) && $y->id_na && $y->rut!=""){
									$message = 'Favor verificar si corresponde a su cartera, si no corresponde contactarse con mesa de operaciones';
								}
								
								if($y->id_mov){
									if(isset($y->id_na) && $y->id_na){
										$res = $this->movimiento_cuenta->get_many_by(array("id_cuenta_na" => $y->id_na));
										if(count($res)>1)
											$link = "estadodiario/index.php/admin/movimientos/listar/0/{$y->id_na}";
									}
									else{
										$res = $this->movimiento_cuenta->get_many_by(array("id_cuenta" => $y->id));
										if(count($res)>1)
											$link = "estadodiario/index.php/admin/movimientos/listar/{$y->id}";
									}
								}
								...
								if($link){
									...
									$cuerpo2.="<tr><td>{$y->rol}</td><td>{$y->tribunal}</td><td>{$y->caratulado}</td>".
											  "<td>{$y->rut}</td><td>{$fecha_etapa}</td><td>{$y->etapa_cuenta}</td>".
											  "<td>{$message}</td><td>{$y->estado}</td><td>{$y->mandante}</td><td>{$procuradorX}</td>".
											  "<td>Consulte aquí: (<a href='{$interno}{$link}' target='_blank'>oficina</a>) ".
											  " (<a href='{$externo}{$link}' target='_blank'>casa</a>)</td></tr>";
								}
								elseif($y->id_mov){
									//echo "<a href='{$y->url}' target='_blank'><img src='{$url_img}'></a>"; die;
									$procuradorX = $y->procurador;
									$cuerpo2.="<tr><td>{$y->rol}</td><td>{$y->tribunal}</td><td>{$y->caratulado}</td>".
											  "<td>{$y->rut}</td><td>{$fecha_etapa}</td><td>{$y->etapa_cuenta}</td><td></td>".
											  "<td>{$y->estado}</td><td>{$y->mandante}</td><td>{$procuradorX}</td>".
											  "<td>{$y->etapa}</td><td>{$y->tramite}</td><td>{$y->descripcion}</td><td>{$y->fecha}</td>".
											  "<td><a href='{$y->url}' target='_blank'>link</a></td></tr>";
								}
								else{
									$procuradorX = $y->procurador;
									$cuerpo2.="<tr><td>{$y->rol}</td><td>{$y->tribunal}</td><td>{$y->caratulado}</td>".
            ...
            $cuerpo2.= "<tr><td>TOTAL</td><td colspan='13'></td><td style='text-align:center;'>{$causas2}</td></tr>";
            $cuerpo3.= "<tr><td>TOTAL</td><td colspan='3'></td><td style='text-align:center;'>{$causas3}</td></tr>";
            $cuerpo4.= "<tr><td>TOTAL</td><td colspan='7'></td><td style='text-align:center;'>{$causas4}</td></tr>";
            $cuerpo5.= "<tr><td>TOTAL</td><td colspan='7'></td><td style='text-align:center;'>{$causas5}</td></tr>";
            ...
            if($find){
				...
				$mail = $this->getPhpMailer($fecha_tramite, $email);
				$arr = explode('-', $key);
                $correos_cc = $this->correo_m->get_many_by(array('id_procurador' => $arr[0]));
                foreach ($correos_cc as $a => $b){
					$correo_cc = $b->correo;
                    $mail->addCC($correo_cc);
                }
				
				//echo $causas1.'-->'.$causas2; die;
                if($apel && !$abog){
					...
					//Cargo la tabla en un html temporal
					$tmpfile = tempnam(sys_get_temp_dir(), 'html');
					file_put_contents($tmpfile, utf8_decode($body1));
					
					$objPHPExcel     = new PHPExcel();
					$excelHTMLReader = PHPExcel_IOFactory::createReader('HTML');
					$excelHTMLReader->loadIntoExisting($tmpfile, $objPHPExcel);
					$objPHPExcel->getActiveSheet()->setTitle('Mis Causas');
					
					$writer = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
					$writer->save('estadodiario.xlsx');
					
					$file = $_SERVER['DOCUMENT_ROOT'].'/estadodiario/estadodiario.xlsx';
					$mail->addAttachment($file, 'estadodiario.xlsx');
					$mail->MsgHTML($body.'<br/>'.$body1);
                    $sent = $mail->send();
					unlink($tmpfile);
					unlink($file);
                    unset($mail);
                    if($causas2){
						...
						//Cargo la tabla en un html temporal
						$tmpfile = tempnam(sys_get_temp_dir(), 'html');
						file_put_contents($tmpfile, utf8_decode($body2));
				...
				elseif($labr && $labr_na){
					...
					//Cargo la tabla en un html temporal
					$tmpfile = tempnam(sys_get_temp_dir(), 'html');
					file_put_contents($tmpfile, utf8_decode($body3));
				...
				elseif($labr && !$labr_na){
					...
					//Cargo la tabla en un html temporal
					$tmpfile = tempnam(sys_get_temp_dir(), 'html');
					file_put_contents($tmpfile, utf8_decode($body4));
				...
				elseif($cobrz){
					...
					//Cargo la tabla en un html temporal
					$tmpfile = tempnam(sys_get_temp_dir(), 'html');
					file_put_contents($tmpfile, utf8_decode($body5));
                ...
                if(!$sent){
                    $no_enviados++;
                    $causas_no_enviadas += intval($causas1) + intval($causas2) + intval($causas3) + intval($causas4);
                } else {
                    $enviados++;
                    $causas_enviadas += intval($causas1) + intval($causas2) + intval($causas3) + intval($causas4);
                }
            }
        }
    ...
    public function getPhpMailer($fecha_tramite, $e){
        $mail = new PHPMailer;        
        $mail->isSMTP();
        ...
        $mail->Subject = 'MOVIMIENTO ESTADO DIARIO '.$fecha_tramite; // .' ('.$e.')'
        return $mail;
    }
	...
	function index($action='',$id=''){
		...
		if ($view=='list'){
			...
			$this->data['enviados'] = $this->data['no_enviados'] = 0;
			$this->data['causa_enviada'] = $this->data['causas_no_enviada'] = 0;
			$envios = $this->reg_correo_m->get_all();
			if($envios){
				$this->data['enviados'] = $envios[0]->enviados;
				$this->data['no_enviados'] = $envios[0]->no_enviados;
				$this->data['causa_enviada'] = $envios[0]->causa_enviada;
	...
	public function descargar(){
		...
		$sheet->SetCellValue ( 'D1', 'CARATULADO' );
		$sheet->SetCellValue ( 'E1', 'ETAPA' );
		$sheet->SetCellValue ( 'F1', 'TRAMITE DESC.' );
		$sheet->SetCellValue ( 'G1', 'TRAMITE' );
		...
		foreach($cuentas as $key => $val){
			...
			$tipo = 'CIVIL';
			if($val->sistema == 'laboral')
				$tipo = 'LABORAL';
			elseif($val->sistema == 'penal')
				$tipo = 'PENAL';
			elseif($val->sistema == 'cobranza')
				$tipo = 'COBRANZA';
			
			$rol = ($val->rol) ? $val->rol : (($val->rit) ? $val->rit : $val->rol_interno);
			$arr = array($rol, $val->tribunal);
			if(!in_array($arr, $no_repetidos)){
				$no_repetidos[] = $arr;
				$sheet->SetCellValue ( 'A' . $i, $tipo );
				$sheet->SetCellValue ( 'B' . $i, $rol );
				$sheet->SetCellValue ( 'C' . $i, $val->tribunal );
				$sheet->SetCellValue ( 'D' . $i, $val->caratulado );
				$sheet->SetCellValue ( 'E' . $i, $val->etapa );
				$sheet->SetCellValue ( 'F' . $i, $val->tramite );
				$sheet->SetCellValue ( 'G' . $i, $val->descripcion );
				$sheet->SetCellValue ( 'H' . $i, $val->rut);
				$sheet->SetCellValue ( 'I' . $i, $fecha_etapa);
				$sheet->SetCellValue ( 'J' . $i, $val->etapa_cuenta);
				$sheet->SetCellValue ( 'K' . $i, $val->estado);
				$sheet->SetCellValue ( 'L' . $i, $val->mandante);
				$i ++;
			}
		}
    ...
    function limpiarbd(){
		...
		$this->cuentas_m->delete_all();
		$this->cuentas_na->delete_all();
		$this->cuentas_aux->delete_all();
		$this->reg_correo_m->delete_all();
		$this->receptor_pjud->delete_all();
		$this->consulta_pjud_m->delete_all();
		$this->movimiento_cuenta->delete_all();
		$abogados = $this->abogados_m->get_all();
		foreach($abogados as $k => $v){
			$datos_abog = array();
			$datos_abog['total_registros_archivo'] = 0;
			$datos_abog['total_registros'] = 0;
			$datos_abog['total_import'] = 0;
			$datos_abog['cruce_gespron'] = 0;
			$datos_abog['total_elim'] = 0;
			$datos_abog['total_match'] = 0;
			$datos_abog['total_na'] = 0;
			$datos_abog['contador'] = 0;
			$datos_abog['distribuidos'] = 0;
	...
	public function revisar_cuentas2($datos=array()){
		...
		if($tribunal_pjud){
			$cuenta = $this->cuentas_m->get_by(array("rol" => $datos['rol'], 'tribunal' => $tribunal_pjud));
			$cuenta_na = $this->cuentas_na->get_by(array("rol" => $datos['rol'], 'tribunal' => $tribunal_pjud));
		}
		if(!$cuenta && !$cuenta_na){
			if($tribunal_pjud)
