...
class Importar extends CI_Controller {
	...
	public function revisar_cuentas($datos=array()){
		...
		$cuenta_na = $this->cuentas_m->get_by(array("rol" => $aux_rol, 'id_tribunal' => $aux_trb));
		...
		if(($cuenta && $cuenta->id_abogado != $datos_cuentas['id_abogado']) || ($cuenta_na && $cuenta_na->id_abogado != $datos_cuentas['id_abogado']) || !($cuenta && $cuenta_na)){
			if($result[0]->rut == ''){
				...
				if($datos_cuentas['id_abogado'] == 1){ //CINTHYA
					$passed = false;
					if($cuenta){
						if($cuenta->id_abogado == 3){ //LESLIE (No se inserta)
							$passed = true;
							$id_abogado = 1;
							$datos_abog = array();
						...
						elseif($cuenta->id_abogado == 4){ //CAROLINA (eliminamos a carolina)
							$this->cuentas_m->delete($cuenta->id);
							$id_abogado = 4;
							$datos_abog = array();
							$abogad = $this->abogados_m->get_by(array("id" => 4));
							$datos_abog['total_elim'] = intval($abogad->total_elim) + 1;
							$this->abogados_m->update($id_abogado, $datos_abog, false, true);
						}
					}
					elseif($cuenta_na){
						if($cuenta_na->id_abogado == 3){ //LESLIE (No se inserta)
							$passed = true;
							$id_abogado = 1;
							$datos_abog = array();
							$abogad = $this->abogados_m->get_by(array("id" => 1));
						...
						elseif($cuenta->id_abogado == 4){ //CAROLINA (eliminamos a carolina)
							$this->cuenta_na->delete($cuenta_na->id);
							$id_abogado = 4;
							$datos_abog = array();
							$datos_abog['total_na'] = intval($abogad->total_na) - 1;
							$this->abogados_m->update($id_abogado, $datos_abog, false, true);
			...
			} // ME QUEDE AQUIIIIIIIII
			//print_r($datos_cuentas); die;
			if($datos_cuentas['id_abogado'] > 1){
				$id_abogado = $datos_cuentas['id_abogado'];
				$arr_abog = array(1, 2, 3);
				$passed = false;
				if($cuenta){
					//echo $id_abogado.'--->'.$cuenta->id_abogado;
					if($id_abogado == 2 && $cuenta->id_abogado == 4){ //ISIS (Quitamos a carolina)
						$na_quitar = false;
						$this->cuentas_m->delete($cuenta->id);
					...
					elseif($id_abogado == 3 && $cuenta->id_abogado == 4){ //LESLIE (Quitamos a carolina)
						$na_quitar = false;
						$this->cuentas_m->delete($cuenta->id);
						$cuenta_na = $this->cuentas_na->get_by(array("rol" => $aux_rol, 'id_tribunal' => $aux_trb));
						//print_r($cuenta_na);
					...
					elseif($id_abogado == 4 && in_array($cuenta->id_abogado, $arr_abog)){ //CAROLINA (No insertamos)
						$na_quitar = false;
						$cuenta_na = $this->cuentas_na->get_by(array("rol" => $aux_rol, 'id_tribunal' => $aux_trb));
						//print_r($cuenta_na);
						if($cuenta_na){
...
//ECHO $jurisdiccionexhorto;
	...
	public function importar_excel_nodo_fullpay($abogado=''){
		...
		$id_abogado = $datos_cuentas['id_abogado'];
		$datos_save = array();
		$datos_save['total_registros'] = $count;
		$datos_save['total_import'] = $this->array_return['cuentas_insert'];
		//$this->abogados_m->save_default($datos_save, $id_abogado);
		$this->abogados_m->update($id_abogado, $datos_save, false, true);
	}
