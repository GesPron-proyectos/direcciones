...
class Roles extends CI_Controller {
	...
	public function Roles() { $this->__construct(); }
	function __construct() {
		parent::__construct();
		$this->load->library ( 'session' );
		
		$action = $this->uri->segment(3);
		if($action == "ejecutar_roles"){
			//INICIAR SESSION PARA EJECUTAR ROLES
			$this->load->helper('security');
			$this->session->set_userdata("usuario_id", 54);
			$this->session->set_userdata("usuario_perfil", 1);
			$this->session->set_userdata("usuario_username", "jvasquez");
			$this->session->set_userdata("logged_in", TRUE);
		}
		elseif(!$this->session->userdata('usuario_id'))
		...
		$this->load->model ( 'movimiento_cuenta_roles');
		$this->load->model ( 'receptor_roles');
		
		
		/*seters*/
		$this->data['current'] = 'roles';
	...
	public function distribuir($action=''){
		$config1 = $this->configuracion_m->get_many_by(array('sistema' => 'na', 'tipoc' => 'C'));

		$where = "sistema != 'na'";
		$config2 = $this->db->select('*')->where($where)->get('configuracion')->result();

		$where = "sistema = 'na' and tipoc != 'C'";
		$config3 = $this->db->select('*')->where($where)->get('configuracion')->result();

		$configs = array_merge($config1, $config2, $config3);

		//print_r($configs); die;

		$cuentas = $ids = array();
		foreach($configs as $key => $value){
			$where = $where_str = $result = array(); $tribunal = $param = 0;
			$procurador = $this->procurador_m->get_by(array('id'=>$value->id_procurador));
			//echo '-->'.$value->id_procurador; 
			$where['nombres'] = trim($procurador->nombre).' '.trim($procurador->apellido);
			if($value->sistema == 'apel'){
				$query = $this->db->select('cta.id AS id,
											cta.rol AS nro_ingreso,
											cta.tribunal AS corte,
											...
											')
								  ->from("0_cuentas cta")
								  ->where(array('seleccionado'=>0, 'sistema'=>'apel'))
								  ->get();
				$result = $query->result();
			}
			else{
				...
				if($value->tipoc){
					$param = 1;
					$where_str = "rol like '{$value->tipoc}-%'";
				}
				...
				if($value->sistema == 'na'){
					if($where['nombres'] != 'Michel Valdivia'){
					...
					elseif(!$param){
						$where_str = "rut IS NULL OR nombres = ''";
					}
					$query = $this->db->select('cta.id AS id,
												cta.id_abogado,
												cta.rol AS rol,
			...
			if($result){
				if($value->id_designado)
					$cuentas[$value->id_designado][] = $result;
				else
					$cuentas[$value->id_procurador][] = $result;
		...
		Php_mailer::enviar_correo($cuentas);
	...
	public function revisar_cuentas($datos=array()){
		...
		$result1 = $this->cae->select('
									...
									tpj.id_tribunal_pjud AS id_tribunal,
									tpje.id_tribunal_pjud AS id_tribunal_ex,
								')
							  ->from("0_cuentas cta")
							  ...
							  ->join("s_tribunales tri", "tri.id = cta.id_tribunal","left")
							  ->join("jurisdiccion ju", "ju.id = cta.id_distrito","left")
							  ->join("s_tribunales trie", "trie.id = cta.id_tribunal_ex","left")
							  ->join("jurisdiccion jue", "jue.id = cta.id_distrito_ex","left")
							  ->join("anio a","a.id=cta.anio")
							  ->join("anio ae","ae.id=cta.anioE","left")
							  ->join("tipo_causa tp","tp.id=cta.letraC")
							  ->join("tipo_causa tpe","tpe.id=cta.letraE","left")
							  ->join("tribunales_pjud tpj","tpj.id_s_tribunales=cta.id_tribunal","left")
							  ->join("tribunales_pjud tpje","tpje.id_s_tribunales=cta.id_tribunal_ex","left")
							  ->where("cta.activo = 'S' and cta.id_mandante in (2, 12) and cta.id_estado_cuenta != 5")		
							  ->group_by('cta.id')						  
                              ->get()
							  ->result();
		...
		if(count($result1) > 0) {
			...
			foreach ($result1 as $key => $value){ 
				$value->sistema='cae-itau';
				$this->db->insert("roles_0_cuentas", $value);
				if($value->letraE != "" && $value->anioE != "" && $value->rolE != "")
					$count_e++;
			}
				
			$count=count($result1);
			$sistema= $this->sistemas_m->get_by(array('sistema'=> 'cae-itau'));
			$data=array();
			$data['total_registros'] = $count;
			$data['total_registros_E'] = $count_e;
			$this->sistemas_m->update ( $sistema->id, $data, TRUE, TRUE );
		...
		$result1 = $this->cae->select('
							  ...
							  ->join("tribunales_pjud tpj","tpj.id_s_tribunales=cta.id_tribunal","left")
							  ->join("tribunales_pjud tpje","tpje.id_s_tribunales=cta.id_tribunal_ex","left")
							  ->where("cta.activo = 'S' and cta.id_mandante = 6 and cta.id_estado_cuenta != 5")		
							  ->group_by('cta.id')
                              ->get()
							  ->result();
		...
		if(count($result1) > 0) {
			...
			$count=count($result1);
			$sistema= $this->sistemas_m->get_by(array('sistema'=> 'cae-scotia'));
			$data=array();
			$data['total_registros'] = $count;
			$data['total_registros_E'] = $count_e;
			$this->sistemas_m->update ( $sistema->id, $data, TRUE, TRUE );
		...
		$result2 = $this->cat->select('
								...
								')
							  ->from("0_cuentas cta")
							  ->join("0_usuarios u","u.id=cta.id_usuario")
							  ->join("2_cuentas_etapas 2ce","2ce.id_cuenta=cta.id", "left")
							  ->join("s_etapas se","se.id=cta.id_etapa", "left")
							  ->join("0_mandantes ma","ma.id=cta.id_mandante", "left")
							  ->join("s_estado_cuenta es","es.id=cta.id_estado_cuenta", "left")
							  ...
							  ->join("tribunales_pjud tpj","tpj.id_s_tribunales=cta.id_tribunal","left")
							  ->join("tribunales_pjud tpje","tpje.id_s_tribunales=cta.id_tribunal_ex","left")
							  ->where("cta.activo  = 'S' and cta.id_mandante != 9  and cta.id_estado_cuenta != 5")
							  ->group_by('cta.id')
                              ->get()
							  ->result();

			 //echo $this->cat->last_query();
			 //die;
		if(count($result2) > 0) {
			//	$consulta = $result->result_array();
			$count_e = 0;
			...
			$count=count($result2);
			$sistema= $this->sistemas_m->get_by(array('sistema'=> 'cat'));
			$data=array();
			$data['total_registros'] = $count;
			$data['total_registros_E'] = $count_e;
			$this->sistemas_m->update ( $sistema->id, $data, TRUE, TRUE );
		...
		$result3 = $this->sup->select('
							  ...
							  ->join("s_etapas se","se.id=cta.id_etapa", "left")
							  ->join("0_mandantes ma","ma.id=cta.id_mandante", "left")
							  ->join("s_estado_cuenta es","es.id=cta.id_estado_cuenta")
							  ->join("0_administradores adm","adm.id=cta.id_procurador", "left")
							  ->join("s_tribunales tri", "tri.id = cta.id_tribunal","left")
							  ->join("jurisdiccion ju", "ju.id = cta.id_distrito","left")
							  ->join("s_tribunales trie", "trie.id = cta.id_tribunal_ex","left")
		...
		if(count($result3) > 0) {
			...
			$today = date('Y-m-d');
			$count=count($result3);
			$sistema= $this->sistemas_m->get_by(array('sistema'=> 'sup'));
			$data=array();
			$data['fecha_gestion'] = $today;
			$data['total_registros'] = $count;
			$data['total_registros_E'] = $count_e;
			$this->sistemas_m->update($sistema->id, $data, TRUE, TRUE);
	...
	function index($action='',$id=''){
		...
		if ($view=='roles_list'){
			...
			$config['base_url'] = site_url().'/admin/roles/index/';
		...
		//  $total_rows = array_merge($query1, $query2); //print_r($total_rows); die;
			$query_total = $this->db->get('roles_0_cuentas cta');					 
			...
			$this->cat = $this->load->database("cat", TRUE);
			$this->sup = $this->load->database("superir", TRUE);
			//$config_cae['base_url'] = site_url().'/admin/roles/index/';
			
			$query_total_cae = $this->cae->from('0_cuentas cta')->where("cta.id_mandante in (2, 12) and cta.activo = 'S'")->get();
				
			$total_rows_cae = $query_total_cae->result();
			$this->data['total_rows_cae'] = count($total_rows_cae);
			
			$query_total_cae_scotia = $this->cae->from('0_cuentas cta')->where("cta.id_mandante = 6 and cta.activo = 'S'")->get();
				
			$total_rows_cae_scotia = $query_total_cae_scotia->result();
			$this->data['total_rows_cae_scotia'] = count($total_rows_cae_scotia);
			
			//QUERY TOTAL CAT
			
			$query_total_cat = $this->cat->from('0_cuentas cta')->join("0_usuarios u","u.id=cta.id_usuario")->where("cta.activo = 'S'")->get();
				
			$total_rows_cat = $query_total_cat->result();
			$this->data['total_rows_cat'] = count($total_rows_cat);
			
			//QUERY TOTAL SUPERIR
			
			$query_total_sup = $this->sup->from('0_cuentas cta') ->where("cta.activo = 'S'")->get();
				
			$total_rows_sup = $query_total_sup->result();
			$this->data['total_rows_sup'] = count($total_rows_sup);
			...
			$this->db->select('
				...
				cta.etapa AS etapa,
				cta.mandante AS mandante,
				cta.estado AS estado,
				cta.nombres AS nombres,
				cta.tribunal AS tribunal,
				cta.jurisdiccion AS jurisdiccion,
				cta.letraC AS letraC,
				cta.anio as anio,
				cta.rol AS rol,
				mc.etapa AS etapa_pjud, 
				mc.tramite AS tramite_pjud,
				mc.descripcion AS descripcion_pjud,
				mc.fecha AS fecha_etapa_pjud,
				mc.url AS url,
				rc.id_cuenta as id_mov
				');
									
			$this->db->join("roles_movimiento_cuenta mc", "cta.id = mc.id_cuenta");
			$this->db->join("receptor_roles rc", "cta.id = rc.id_cuenta");
			$this->db->where($where);
			$this->db->like($like);
			//$this->db->order_by($order_by);
			$this->db->group_by("cta.id");
			$query =  $this->db->get('roles_0_cuentas cta', $config['per_page'], $this->data['current_pag']);
			
			$list = $query->result();

			$a=$this->sistemas_m->get_all();
			$this->data['sistemas'] = $a;
			
			$this->data['todos'] = false;
			$this->data['nuevo'] = true;
			if(intval($config['total_rows']) > 0){
				$todos = true;
				$ayerd = date('Y-m-d', strtotime('-1 day'));
				$fecha = $compara = '';
				foreach($a as $key => $value){
					if($value->fecha_gestion == '0000-00-00')
						$todos = false;
					else{
						$fecha_aux = date('Y-m-d', strtotime($value->fecha_gestion));
						if($fecha)
							$compara = $this->comparaFechas($fecha, $fecha_aux);
						else
							$fecha = $fecha_aux;
						if($compara == 'menor')
							$fecha = $fecha_aux;
			...
			$this->db->where("causa_reservada = 1 or causa_reservada_E = 1");
			$query =  $this->db->get('roles_0_cuentas cta');
			$cant1 = $query->result();
			
			$reservadas = intval($cant1[0]->cant);
			$this->data['reservadas'] = $reservadas;
			
			//Contar las reservadas
			$this->db->select('count(*) as cant');
			$this->db->where("no_encontrado_pjud = 1 or no_encontrado_pjud_E = 1");
			$query =  $this->db->get('roles_0_cuentas cta');
			$cant = $query->result();
			
			$no_encontradas = intval($cant[0]->cant);
			$this->data['no_encontradas'] = $no_encontradas;

			$this->db->start_cache();
			$this->db->stop_cache();
			
			//$this->data['lists'] = array_merge($list1, $list2,$list3); //print_r($this->data['lists']); die;
			...
			if (!$this->show_tpl){ 
				$this->data['plantilla'] = 'cuentas/roles_list_tabla'; 
	...
	public function descargar(){
		...
		$sheet->SetCellValue ( 'B1', 'TRIBUNAL' );
		$sheet->SetCellValue ( 'C1', 'CARATULADO' );
		$sheet->SetCellValue ( 'D1', 'ETAPA' );
		$sheet->SetCellValue ( 'E1', 'TRAMITE DESC.' );
		$sheet->SetCellValue ( 'F1', 'TRAMITE' );
		$sheet->SetCellValue ( 'G1', 'RUT' );
		$sheet->SetCellValue ( 'H1', 'FECHA ETAPA' );
		...
		foreach($cuentas as $key => $val) {
			...
			$sheet->SetCellValue ( 'A' . $i, $val->rol );
			$sheet->SetCellValue ( 'B' . $i, $val->tribunal );
			$sheet->SetCellValue ( 'C' . $i, $val->caratulado );
			$sheet->SetCellValue ( 'D' . $i, $val->etapa );
			$sheet->SetCellValue ( 'E' . $i, $val->tramite );
			$sheet->SetCellValue ( 'F' . $i, $val->descripcion );
			$sheet->SetCellValue ( 'G' . $i, $val->rut);
			$sheet->SetCellValue ( 'H' . $i, $fecha_etapa);
			$sheet->SetCellValue ( 'I' . $i, $val->etapa_cuenta);
			$sheet->SetCellValue ( 'J' . $i, $val->estado);
			$sheet->SetCellValue ( 'K' . $i, $val->mandante);
			$i ++;
		}
		...
		header ( 'Content-Disposition: attachment; filename=no_seleccionados_' . date ( 'd/m/Y' ) . '_' . date ( 'His' ) . '.xls' );
		header ( 'Expires: 0' );
	...
	function limpiarbd(){
		...
		$this->roles_m->delete_all();
		$this->movimiento_cuenta_roles->delete_all();
		for ($i=1; $i <= 4; $i++) { 
			$sistema = $this->sistemas_m->get_by(array("id" => $i));
			if($sistema){
				$datos_sist = array();
				$datos_sist['total_registros'] = 0;
				$datos_sist['total_registros_E'] = 0;
				$datos_sist['total_import'] = 0;
				$datos_sist['total_elim'] = 0;
				$datos_sist['total_match'] = 0;
				$datos_sist['total_match_E'] = 0;
				$datos_sist['total_na'] = 0;
				$datos_sist['total_na_E'] = 0;
				$datos_sist['distribuidos'] = 0;
				$datos_abog['fecha_gestion'] = '0000-00-00';
				$this->sistemas_m->update($i, $datos_sist, false, true);
			}
		}
		echo json_encode(array('result' => 1));
		die();
	...
	function cruce_pjud(){
		...
		$this->db->select('
			...
			cta.sistema
			');
		$this->db->where("cruce_pjud = 0 and causa_reservada = 0 and sistema='{$sistema}' and rol is not null and letraC is not null and anio is not null and id_tribunal is not null");
	...
	function cruce_pjud_e(){
		...
		$this->db->select('
			...
			cta.sistema
			');
		$this->db->where("cruce_pjud_E = 0 and causa_reservada_E = 0 and sistema='{$sistema}' and rolE is not null and letraE is not null and anioE is not null and id_tribunal_ex is not null");
		$this->db->limit($limit, $start);
		$this->db->group_by("cta.id");
		$query = $this->db->get('roles_0_cuentas cta');
		$data2 = $query->result();
	...
	function pjud($data, $opt){
		...
		foreach($data as $key => $value){
			...
			$rol = trim($value->rol);
			$anio = trim($value->anio);
			$letraC = trim($value->letraC);
			//$rol = explode('-', $rol);
			//print_r($rol);
			//die;
			$aux = $url;
			$url = $url.'ROL_Causa='.$rol.'&TIP_Causa='.$letraC.'&ERA_Causa='.$anio.'&COD_Tribunal='.$value->id_tribunal.'&TIP_Informe=1&';
			//echo $url; die;
			$arr_idc = array();
			...
			foreach($arr_idc as $idc){
				$idt = 1; //Principal
				if($idc == "29148726" || $idc == "32799974" || $idc == "32867169")
					$idt = 116; //Concursal
				elseif($idc == "32715383" || $idc == "32989560")
					$idt = 19; // Apremio
				elseif($idc == "32586681")
				...
				if(strpos($results, 'Cannot find bean under name') !== false){
					if($opt==1){
						...
						$this->roles_m->update($value->id, $datos_cuenta, false, true);
					}
				}
				else{
					libxml_use_internal_errors(true);
					...
					if($div1){
						...
						if($fecha_viernes)
							...
							$fecha_ayer = date('Y-m-d', strtotime('-2 day'));
						foreach ($rows as $row){
							...
							if($fecha_viernes){
								$compara1 = $this->comparaFechas($fecha, $fecha_viernes);
								$compara2 = $this->comparaFechas($fecha, $fecha_sabado);
								if($compara1 == 'igual' || $compara2 == 'igual'){
									$tiene_mov = true;
								...
								//echo $fecha."--->".$fecha_ayer;
								$compara = $this->comparaFechas($fecha, $fecha_ayer);
								if($compara == 'igual'){
									$tiene_mov = true;
									$etapa = trim($cols->item(3)->nodeValue);
									$tramite = trim($cols->item(4)->nodeValue);
						...
						if($opt==1 && $tiene_mov){
							...
							$id_mov = $this->movimiento_cuenta_roles->insert($data_save, false, true);
							//ACTUALIZO CRUCE PJUD EN CUENTA
							$datos_cuenta = array();
							$datos_cuenta['tiene_mov'] = 1;
							$datos_cuenta['cruce_pjud'] = 1;
							$this->roles_m->update($value->id, $datos_cuenta, false, true);
							...
							if(!$param){
								$sistema = $value->sistema;
								$datos_sistema = array();
								$sistem = $this->sistemas_m->get_by(array("sistema" => $sistema));
								$datos_sistema['total_match'] = intval($sistem->total_match) + 1;
								$this->sistemas_m->update($sistem->id, $datos_sistema, false, true);
							}
						}
						elseif($opt==1){
							//ACTUALIZO CRUCE PJUD EN CUENTA
					...
					elseif(strpos($results, 'Causa no existe en el sistema') !== false){
						if($opt==1){
							//ACTUALIZO CRUCE PJUD EN CUENTA
							$datos_cuenta = array();
							$datos_cuenta['no_encontrado_pjud'] = 1;
							$this->roles_m->update($value->id, $datos_cuenta, false, true);
						}
					}
					elseif(strpos($results, 'Problema de Sistema') !== false){	
							$datos_cuenta = array();
							$datos_cuenta['no_encontrado_pjud'] = 1;
							$this->roles_m->update($value->id, $datos_cuenta, false, true);
						...
						if(!$param){
							...
							$datos_sistema = array();
							$sistem = $this->sistemas_m->get_by(array("sistema" => $sistema));
							$datos_sistema['total_na'] = intval($sistem->total_na) + 1;
							$this->sistemas_m->update($sistem->id, $datos_sistema, false, true);
							//ACTUALIZO CRUCE PJUD EN CUENTA
							$datos_cuenta = array();
							$datos_cuenta['cruce_pjud'] = 1;
		...
		//die("termino el ciclo");
	}
	
	function pjudE($data, $opt){
		$url = "https://civil.pjud.cl/CIVILPORWEB/ConsultaDetalleAtPublicoAccion.do?TIP_Consulta=1&";
		define('USER_AGENT', 'Mozilla / 5.0 (Windows NT 5.1) AppleWebKit / 537.36 (KHTML, como Gecko) Chrome / 35.0.2309.372 Safari / 537.36');
        define('COOKIE_FILE', 'cookie.txt');
        $count = 0;
		foreach($data as $key => $value){
			...
			$rol = trim($value->rolE);
			$arr = explode('-', $rol);
			$id_tribunal_ex = $value->id_tribunal_ex;
			//print_r($rol); die;
			if(count($arr) == 3){
				$excluir = true;
			} else{
				$letra = trim($value->letraE);
				$rol = trim($value->rolE);
				$anio = trim($value->anioE);
			}
			//print_r($rol);
			//die;
			if(!$excluir){
				$aux = $url;
				$url = $url.'ROL_Causa='.$rol.'&TIP_Causa='.$letra.'&ERA_Causa='.$anio.'&COD_Tribunal='.$id_tribunal_ex.'&TIP_Informe=1&';
				//echo $url; die;
				$arr_idc = array();
				$arr_idc = $this->getURL($url);
				$param = 0;
				foreach($arr_idc as $idc){
					...
					if(strpos($results, 'Cannot find bean under name') !== false){
						if($opt==1){
							//ACTUALIZO CRUCE PJUD EN CUENTA
							$datos_cuenta = array();
							$datos_cuenta['causa_reservada_E'] = 1;
							$datos_cuenta['cruce_pjud_E'] = 1;
							$this->roles_m->update($value->id, $datos_cuenta, false, true);
						}
					}
					else{
						...
						if($div1){
							...
							foreach ($rows as $row){
								...
								if($fecha_viernes){
									...
									$compara = $this->comparaFechas($fecha, $fecha_ayer);
									//echo $letra."-".$rol."-"."--".$anio."--".$fecha."--".$fecha_ayer."--".$compara."--";
									if($compara == 'igual'){
										$tiene_mov = true;
										$etapa = trim($cols->item(3)->nodeValue);
										$tramite = trim($cols->item(4)->nodeValue);
							...
							if($opt==1 && $tiene_mov){
								...
								if(!$param){
									...
									$datos_sistema = array();
									$sistem = $this->sistemas_m->get_by(array("sistema" => $sistema));
									$datos_sistema['total_match_E'] = intval($sistem->total_match_E) + 1;
									$this->sistemas_m->update($sistem->id, $datos_sistema, false, true);
								}
							}
							elseif($opt==1){
							...
							if($div2){
								...
								foreach($rows as $row) { 
									...
									if($fecha)
										$compara = $this->comparaFechas($fecha, $fecha_hoy);
									//echo $fecha."--->".$fecha_hoy."--->".$compara; die;
									if($compara == 'igual'){
										$cuaderno = trim($cols->item(0)->nodeValue);
										$dato_ret = trim($cols->item(1)->nodeValue);
										$estadore = trim($cols->item(2)->nodeValue);
						...
						elseif(strpos($results, 'Causa no existe en el sistema') !== false && $id_tribunal_ex != ""){
							if($opt==1){
								//ACTUALIZO CRUCE PJUD EN CUENTA
								$datos_cuenta = array();
								$datos_cuenta['no_encontrado_pjud_E'] = 1;
								$this->roles_m->update($value->id, $datos_cuenta, false, true);
							}
						}
						elseif(strpos($results, 'Problema de Sistema') !== false && $id_tribunal_ex != ""){
							$datos_cuenta = array();
							$datos_cuenta['no_encontrado_pjud_E'] = 1;
							$this->roles_m->update($value->id, $datos_cuenta, false, true);
						}
							...
							if(!$param){
								...
								$datos_sistema = array();
								$sistem = $this->sistemas_m->get_by(array("sistema" => $sistema));
								$datos_sistema['total_na_E'] = intval($sistem->total_na_E) + 1;
								$this->sistemas_m->update($sistem->id, $datos_sistema, false, true);
								//ACTUALIZO CRUCE PJUD EN CUENTA
								$datos_cuenta = array();
								$datos_cuenta['cruce_pjud_E'] = 1;
