...
class Importar extends CI_Controller {
		...
		if (count($usuario) == '1' ){
	     $idusuario=$usuario->id;		
			 ...
			 if ($idusuario != '') {
				...
				$arreglo_usuario = array_merge ( $arreglo_usuario, array ('fecha_crea' => date ( 'Y-m-d H:i:s' ), 'ip_crea' => $this->input->ip_address () ) );
	...
	//CUENTAS 
	public function revisar_cuentas_tanner($idmandante,$idusuario,$datos=array(),$idprocurador, $idoperacion){
		$idcuenta = '';
		
		//echo $datos['fecha_asignacion'].'<br>';
		//die();
		if (array_key_exists ( 'id_procurador', $datos ) && $datos ['id_procurador'] != '') {
			$datos_cuentas ['id_procurador'] = $idprocurador;
		}
		
		if ($idmandante != '') {
			$datos_cuentas ['id_mandante'] = $idmandante;
		}
		
		if ($idusuario != '') {
           $datos_cuentas ['id_usuario'] = $idusuario;
		}
		
		if (array_key_exists ( 'id_tribunal', $datos ) && $datos ['id_tribunal'] != '') {
			$datos_cuentas ['id_tribunal'] = $datos ['id_tribunal'];
		}

		if (array_key_exists ( 'id_distrito', $datos ) && $datos ['id_distrito'] != '') {
			$datos_cuentas ['id_distrito'] = $datos ['id_distrito'];
		}

		print_r($datos);
		echo "<br>";
		
		if (array_key_exists ( 'id_tribunal_ex', $datos ) && $datos ['id_tribunal_ex'] != '') {
			$datos_cuentas ['id_tribunal_ex'] = $datos ['id_tribunal_ex'];
		}	
		if (array_key_exists ( 'id_distrito_ex', $datos ) && $datos ['id_distrito_ex'] != '') {
			$datos_cuentas ['id_distrito_ex'] = $datos ['id_distrito_ex'];
		}
		if (array_key_exists ( 'rolE', $datos ) && $datos ['rolE'] != '') {
			$datos_cuentas ['rolE'] = $datos ['rolE'];
		}

		
		if (array_key_exists ( 'fecha_asignacion', $datos ) && $datos ['fecha_asignacion'] != '') {
			//echo $datos ['fecha_asignacion'] . 'adentrooooooooooooooooooooooooooo' . '<br>';
			$datos_cuentas ['fecha_asignacion'] = utf8_encode ( $datos ['fecha_asignacion'] );
		}
		if (array_key_exists ( 'rol', $datos ) && $datos ['rol'] != '') {
			$datos_cuentas ['rol'] = $datos ['rol'];
		}
		if (array_key_exists ( 'id_estado_cuenta', $datos ) && $datos ['id_estado_cuenta'] != '') {
			
			$datos_cuentas ['id_estado_cuenta'] = $datos ['id_estado_cuenta'];
		}else{
			//$datos_cuentas ['id_estado_cuenta'] = '1';
		}
		
		$tipo_demanda = '';
		
		if (array_key_exists ( 'tipo_demanda', $datos ) && $datos ['tipo_demanda'] != '') {
			$datos_cuentas ['tipo_demanda'] = $datos ['tipo_demanda'];
			$tipo_demanda = $datos ['tipo_demanda'];
		}
		$exorto = '';
		if (array_key_exists ( 'exorto', $datos ) && $datos ['exorto'] != '') {
			$datos_cuentas ['exorto'] = $datos ['exorto'];
			$exorto = $datos ['exorto'];
		}
		
		$datos_cuentas ['operacion'] = $idoperacion;
		...
		if($this->data['nodo']->nombre=='fullpay'){
			...
			$cuenta = $this->cuentas_m->get_by ( array ('id_usuario' => $idusuario, 'id_mandante' => $idmandante ,'operacion'=>$idoperacion) );
		} else {
		...
		if ($idcuenta != '') {
			...
			$idcuenta = $this->cuentas_m->insert ( $datos_cuentas+array('fecha_escritura_personeria' => ''), TRUE, TRUE );
			$this->array_return['cuentas_insert']++;
			$datos_cuentas_historial=array();
			$datos_cuentas_historial['observaciones'] = 'Cuenta creada de archivo excel';	
			$datos_cuentas_historial['fecha'] = date("Y-m-d H:i:s");
			$datos_cuentas_historial['id_cuenta'] = $idcuenta; 
			$this->revisar_historial($idcuenta,$datos_cuentas_historial);
		}
		die();
		return $idcuenta;
	 }
	...
	//CUENTAS 
	public function revisar_cuentas($idmandante,$idusuario,$datos=array(),$idprocurador){
		$idcuenta = '';
		...
		if($this->data['nodo']->nombre=='fullpay'){
			...
			$cuenta = $this->cuentas_m->get_by ( array ('id_usuario' => $idusuario, 'id_mandante' => $idmandante ,'exorto'=>$exorto,'tipo_demanda'=>$tipo_demanda ) );
		} else {
			$cuenta = $this->cuentas_m->get_by ( array ('id_usuario' => $idusuario, 'id_mandante' => $idmandante ) );
		}
	...
	public function revisar_historial($idcuenta,$datos=array()){
		$idcuenta = '';
		$idhistorial = '';
	// HISTORIAL CUENTA 
     if ($idcuenta != '') {
			$datos = array_merge ( $datos, array ('fecha_mod' => date ( 'Y-m-d H:i:s' ), 'ip_mod' => $this->input->ip_address () ) );
			$this->cuentas_historial_m->update ( $idcuenta, $datos, TRUE, TRUE );
		} else {
			$datos = array_merge ( $datos, array ('fecha_crea' => date ( 'Y-m-d H:i:s' ), 'ip_crea' => $this->input->ip_address () ) );
			$this->cuentas_historial_m->insert ( $datos, TRUE, TRUE );
		}
		return $idhistorial;
	 }
...
//ECHO $jurisdiccionexhorto;
	...
	public function importar_excel_nodo_fullpay(){
		...
		for($i = 2; $i <= $rowcount; $i ++) {
			...
			$telefono = trim ( $excel->val ( $i, 'F', 0 ) );
			$celular = utf8_encode ( trim ( $excel->val ( $i, 'G', 0 ) ) );
			$direccion = trim ( $excel->val ( $i, 'H', 0 ) );
			$numero_direccion = trim ( $excel->val ( $i, 'I', 0 ) );
			$villa = trim ( $excel->val ( $i, 'J', 0 ) );
			$comuna = trim ( $excel->val ( $i, 'K', 0 ) );
			$comuna =  utf8_encode ($comuna);
			$ciudad = trim ( $excel->val ( $i, 'L', 0 ) );
			
			$direccion = $direccion . ' ' . $numero_direccion . ' ' . $villa;
			$fecha_asignacion = trim ( $excel->val ( $i, 'M', 0 ) );
			if ($fecha_asignacion != '') {
				$fecha_asignacion = date ( "Y-m-d", strtotime ( $fecha_asignacion ) );
			}
			$tipo_dda = trim ( $excel->val ( $i, 'N', 0 ) );
			$exhorto = trim ( $excel->val ( $i, 'O', 0 ) );
			$rol = trim ( $excel->val ( $i, 'P', 0 ) );
			$tribunal = trim ( $excel->val ( $i, 'Q', 0 ) );
			
			$jurisdiccion = trim ( $excel->val ( $i, 'R', 0 ) );
			
			$estado = trim ( $excel->val ( $i, 'S', 0 ) );
			$procurador = trim ( $excel->val ( $i, 'T', 0 ) );		
			
			$rolE = trim ( $excel->val ( $i, 'U', 0 ) );
			$tribunalE = trim ( $excel->val ( $i, 'V', 0 ) );
			$jurisdiccionE = trim ( $excel->val ( $i, 'W', 0 ) );
			
			
			//pagare1
			$n_pagare1 = trim ( $excel->val ( $i, 'X', 0 ) );
			$monto_pagare1 = trim ( $excel->val ( $i, 'Y', 0 ) );
			$fecha_asignacion1 = trim ( $excel->val ( $i, 'Z', 0 ) );
			if ($fecha_asignacion1 != '') {
				$fecha_asignacion1 = date ( "Y-m-d", strtotime ( $fecha_asignacion1 ) );
			}
			$fecha_vencimiento1 = trim ( $excel->val ( $i, 'A1', 0 ) );
			if ($fecha_vencimiento1 != '') {
				$fecha_vencimiento1 = date ( "Y-m-d", strtotime ( $fecha_vencimiento1 ) );
			}
			//pagare2
			$n_pagare2 = trim ( $excel->val ( $i, 'AB', 0 ) );
			$monto_pagare2 = trim ( $excel->val ( $i, 'AC', 0 ) );
			$fecha_asignacion2 = trim ( $excel->val ( $i, 'AD', 0 ) );
			if ($fecha_asignacion2 != '') {
				$fecha_asignacion2 = date ( "Y-m-d", strtotime ( $fecha_asignacion2 ) );
			}
			$fecha_vencimiento2 = trim ( $excel->val ( $i, 'AE', 0 ) );
			if ($fecha_vencimiento2 != '') {
				$fecha_vencimiento2 = date ( "Y-m-d", strtotime ( $fecha_vencimiento2 ) );
			}
			//pagare3
			$n_pagare3 = trim ( $excel->val ( $i, 'AF', 0 ) );
			$monto_pagare3 = trim ( $excel->val ( $i, 'AG', 0 ) );
			$fecha_asignacion3 = trim ( $excel->val ( $i, 'AH', 0 ) );
			if ($fecha_asignacion3 != '') {
				$fecha_asignacion3 = date ( "Y-m-d", strtotime ( $fecha_asignacion3 ) );
			}
			$fecha_vencimiento3 = trim ( $excel->val ( $i, 'AI', 0 ) );
			if ($fecha_vencimiento3 != '') {
				$fecha_vencimiento3 = date ( "Y-m-d", strtotime ( $fecha_vencimiento3 ) );
			}
			//pagare4
			$n_pagare4 = trim ( $excel->val ( $i, 'AJ', 0 ) );
			$monto_pagare4 = trim ( $excel->val ( $i, 'AK', 0 ) );
			$fecha_asignacion4 = trim ( $excel->val ( $i, 'AL', 0 ) );
			if ($fecha_asignacion4 != '') {
				$fecha_asignacion4 = date ( "Y-m-d", strtotime ( $fecha_asignacion4 ) );
			}
			$fecha_vencimiento4 = trim ( $excel->val ( $i, 'AM', 0 ) );
			if ($fecha_vencimiento4 != '') {
				$fecha_vencimiento4 = date ( "Y-m-d", strtotime ( $fecha_vencimiento4 ) );
			}
			...
			if ($idmandante != '') {
				...
				//INICIO JURISDICCION
					...
					$datos_tribunal = array ();
					$datos_tribunal ['abr'] = $jurisdiccion;
					$jurisdiccion_ = $this->revisar_distrito_tanner ( $datos_tribunal );
				...
				// INICIO TRIBUNAL			
					...
					$datos_tribunal = array ();
					$datos_tribunal ['abr'] = $tribunal;
					$datos_tribunal ['jurisdiccion'] = $jurisdiccion_;
					$idtribunal_ = $this->revisar_tribunales_tanner ( $datos_tribunal );				
				// FIN TRIBUNAL
				
				// DATOS CUENTA
				$datos_cuentas = array ();
				if ($idtribunal_ != '') {
					$datos_cuentas ['id_tribunal'] = $idtribunal_;
				}
				if ($idtribunal_ != '') {
					$datos_cuentas ['id_distrito'] = $jurisdiccion_;
				}
				...
				$jurisdiccionEx = '';
				$datos_tribunal = array ();
				$datos_tribunal ['abr'] = $jurisdiccionE;
				$jurisdiccionEx = $this->revisar_distrito_tanner ( $datos_tribunal );
				// FIN TRIBUNAL		
				//echo $this->db->last_query();	
				//die();
				// INICIO TRIBUNAL
				$tribunalEx = '';
				$datos_tribunal = array ();
				$datos_tribunal ['abr'] = $tribunalE;
				$datos_tribunal ['jurisdiccion'] = $jurisdiccionEx;
				$tribunalEx = $this->revisar_tribunales_tanner ( $datos_tribunal );
                ...
                //DATOS EXHORTO
                if ($tribunalEx != '') {
					$datos_cuentas ['id_tribunal_ex'] = $tribunalEx;
				}
				if ($jurisdiccionEx != '') {
					$datos_cuentas ['id_distrito_ex'] = $jurisdiccionEx;
				}
				$datos_cuentas ['rolE'] = $rolE;
				...
				if ($idcuenta != '') {
					...
					if ($monto_pagare3!=''){
						...
						$datos_pagare = array ();
						$datos_pagare ['n_pagare'] = $n_pagare3;
						$datos_pagare ['monto_deuda'] = $monto_pagare3;
						$datos_pagare ['fecha_asignacion'] = $fecha_asignacion3;
						$datos_pagare ['fecha_vencimiento'] = $fecha_vencimiento3;
						$this->revisar_pagare ( $idcuenta, $datos_pagare, $idmandante );
					}
					if ($monto_pagare4!=''){
						if ($n_pagare4==''){
						...
						$datos_pagare = array ();
						$datos_pagare ['n_pagare'] = $n_pagare4;
						$datos_pagare ['monto_deuda'] = $monto_pagare4;
						$datos_pagare ['fecha_asignacion'] = $fecha_asignacion4;
						$datos_pagare ['fecha_vencimiento'] = $fecha_vencimiento4;
						$this->revisar_pagare ( $idcuenta, $datos_pagare, $idmandante );
					}
				}
			}
