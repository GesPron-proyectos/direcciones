...
if ($this->form_validation->run() == TRUE){
	if ($this->input->post('monto_deuda')!=''){
		...
		$this->db->query('UPDATE 0_cuentas SET monto_pagado_new = monto_pagado_new + ('.$this->input->post("monto_deuda").') WHERE id='.$id );
	}
...
//###########		
		 ...
		 */
			$query =$this->db->select('
			 ce.id AS id,
			 ce.id_etapa AS id_etapa, ce.obs_administrador, ce.observaciones, ce.fecha_etapa AS fecha_etapa, etapa.etapa AS etapa, adm.nombres AS nombres, adm.apellidos AS apellidos, ce.id AS field_categoria')
	...
	function index($action='',$id='') {
		...
		if ($view=='list'){
			...
			if (isset($_REQUEST['id_estado_cuenta']) && $_REQUEST['id_estado_cuenta']!='' && $_REQUEST['id_estado_cuenta']>='0'){ 
	    	  ...
	    	  } elseif (isset($_REQUEST['id_estado_cuenta']) && $_REQUEST['id_estado_cuenta']>0){
	    		$where["cta.id_estado_cuenta"] = $_REQUEST['id_estado_cuenta'];
	    	if ($config['suffix']!=''){ $config['suffix'].='&';}
    			$config['suffix'].= 'id_estado_cuenta='.$_REQUEST['id_estado_cuenta'];
				//elseif (count($_REQUEST)==0) {
				//$_REQUEST['id_estado_cuenta'] = 1;
				//$where["cta.id_estado_cuenta = 1 OR cta.id_estado_cuenta = 5"];
				//if ($config['suffix']!=''){ $config['suffix'].='&';}
				//$config['suffix'].= 'id_estado_cuenta=1';
            }
	    	*/
			...
			/*$query_total = 
	    		$this->db->where("cta.activo","S")
						 ...
						 ->join("s_tribunales disE", "disE.id = cta.id_distrito_ex","left")
						 ->join("pagare pa", "pa.idcuenta = cta.id","left")
						 ->join("2_cuentas_bienes 2cb", "2cb.id_cuenta = cta.id","left")
			...
			,cta.id_tribunal AS id_tribunal, pa.n_pagare as npage, operacion');
			$this->db->join("0_usuarios usr", "usr.id = cta.id_usuario");
			$this->db->join("0_mandantes mand", "mand.id = cta.id_mandante","left")	;
			$this->db->join("s_tribunales tri", "tri.id = cta.id_tribunal","left");
			$this->db->join("s_tribunales tribu", "tribu.id = cta.id_distrito","left");
	...
	function reporte($tipo = 'pagos',$param = ''){
		...
		foreach ($a as $obj) {$this->data['estados_cuenta'][$obj->id] = $obj->estado;}
			...
			$select_export = 'cta.id AS id, adm.nombres AS nombres, adm.apellidos AS apellidos, mand.razon_social AS razon_social,mand.clase_html AS clase_html,mand.codigo_mandante AS codigo_mandante,sec.estado AS estado,pagos.fecha_pago AS fecha_pago, pagos.monto_pagado AS monto_pagado, cta.monto_deuda AS monto_deuda, usr.nombres AS usr_nombres, usr.ap_pat AS usr_ap_pat, usr.ap_mat AS usr_ap_mat, usr.rut AS rut, pagos.n_comprobante_interno AS n_comprobante_interno';
	    ...
	    if ($tipo == 'etapas'){
	      ...
	      //  $this->output->enable_profiler(TRUE);
			...
			$sql_select =
    			" SELECT ". 
    			"    ce.*, mand.razon_social,mand.clase_html AS clase_html,usr.rut,  adm.apellidos AS amd_apellido, adm.nombres AS adm_nombres, ".
				"    comu.nombre AS comuna,cta.id AS idcuenta,cta.activo AS activo, cta.id_mandante, cta.posicion AS posicion, ".
				"    cta.publico AS publico,cta.monto_deuda AS monto_deuda,cta.rol AS rol, estado.estado AS estado, etapas.etapa AS etapa, ".
				"    mand.razon_social,mand.codigo_mandante AS codigo_mandante, trib.tribunal AS tribunal, dist.tribunal AS distrito, usr.ap_mat AS usr_ap_mat, ".
    			...
    			"	 etapas.dias_alerta AS dias_alerta,DATEDIFF(NOW(),ce.fecha_etapa) as duracion, ".
				"    usr.direccion_numero AS direccion_numero, usr.nombres AS usr_nombres, usr.rut AS rut,trib_com.tribunal as tribunal_padre_comuna";
	...
	public function cuenta_exportar(){
	    ...
	    if ($this->input->get_post('rango')!=''){
    		...
    		$from = date("Y-m-d", strtotime(trim($date[0])));
			$to = date("Y-m-d", strtotime(trim($date[1])));
			$sql_where.=" AND (`cet`.`fecha_etapa` BETWEEN '".$from." 00:00:00' AND '".$to." 23:59:59')";
			
    	} else {
    		
		    if( $p_day = $this->input->get_post('fecha_etapa_day') ){
	    		$sql_where .= " AND DAY(cet.fecha_etapa) = '".$p_day."'";
	    		//$bind_values[] = $p_day;
	    		$suffix['fecha_etapa_day'] =  $p_day;	
	    	}
	    	if( $p_mes = $this->input->get_post('fecha_etapa_month') ){
	    		$sql_where .= " AND MONTH(cet.fecha_etapa) = '".$p_mes."'";
	    		//$bind_values[] = $p_mes;
	    		$suffix['fecha_etapa_month'] =  $p_mes;	
	    	}
	    	if( $p_ano = $this->input->get_post('fecha_etapa_year') ){
	    		$sql_where .= " AND YEAR(cet.fecha_etapa) = '".$p_ano."'";
	    		//$bind_values[] = $p_ano;
	    		$suffix['fecha_etapa_year'] =  $p_ano;	
	    	}
    	}
	    ...
	    if ($this->input->get_post('modo') == "ultima") {
	    	...
	    	$sql_body = 
					...
					"    GROUP BY id_cuenta ".
					" ) ce2 ".
					" JOIN `2_cuentas_etapas` cet ON ce2.id_cuenta = cet.id_cuenta AND ce2.last_fecha = cet.fecha_etapa ".
	...
	public function exportador_cuentas_fullpay() {
		...
		$excel->setActiveSheetIndex ( 0 );
		$sheet = $excel->getActiveSheet ();
		$sheet->SetCellValue ( 'A1', 'N° OPERACION' );
		$sheet->SetCellValue ( 'B1', 'FECHA ASIGNACION' );		
		$sheet->SetCellValue ( 'C1', 'MANDANTE' );
		$sheet->SetCellValue ( 'D1', 'DEUDOR' );
		$sheet->SetCellValue ( 'E1', 'RUT' );
		$sheet->SetCellValue ( 'F1', 'COMUNA' );
		
		$sheet->SetCellValue ( 'G1', 'TRUBUNAL' );
		$sheet->SetCellValue ( 'H1', 'JURISDICCION' );
		$sheet->SetCellValue ( 'I1', 'ROL' );
		$sheet->SetCellValue ( 'J1', 'ETAPA' );
		$sheet->SetCellValue ( 'K1', 'FECHA ETAPA' );		
		$sheet->SetCellValue ( 'L1', 'DÍAS ULTIMA ETAPA' );
		$sheet->SetCellValue ( 'M1', 'TRIBUNAL EXHORTO' );
		$sheet->SetCellValue ( 'N1', 'DISTRITO EXHORTO' );
		$sheet->SetCellValue ( 'O1', 'ROL EXHORTO' );
...
//Fin Nuevo Orden
		...
		foreach ( $cuentas as $key => $val ) {
			...
			$fecha_crea_etapa  = date('d-m-Y',strtotime($val->fecha_crea_etapa));
		    $fecha_crea = date('d-m-Y',strtotime($val->fecha_crea));
		    $ultimo_dia = date('d-m-Y',strtotime($val->ultimo_dia));          
			
		    $sheet->SetCellValue ( 'A' . $i, $val->operacion );
			$sheet->SetCellValue ( 'B' . $i, $fecha_crea );		
			$sheet->SetCellValue ( 'C' . $i, $val->codigo_mandante );
			$sheet->SetCellValue ( 'D' . $i, $val->nombres.' '.$val->ap_pat.' '.$val->ap_mat);
			$sheet->SetCellValue ( 'E' . $i, $val->rut );
			$sheet->SetCellValue ( 'F' . $i, $val->nombre_comuna );			
			$sheet->SetCellValue ( 'G' . $i, $val->distrito);
			$sheet->SetCellValue ( 'H' . $i, $val->tribunal );
			//DistritoE
			
			$sheet->SetCellValue ( 'I' . $i, $val->rol);
			$sheet->SetCellValue ( 'J' . $i, $val->etapa);
			$sheet->SetCellValue ( 'K' . $i, $val->fecha_etapa);
			$sheet->SetCellValue ( 'L' . $i, $val->ultimo_dia );
			$sheet->SetCellValue ( 'M' . $i, $val->tribunalE );
			$sheet->SetCellValue ( 'N' . $i, $val->DistritoE );
			$sheet->SetCellValue ( 'O' . $i, $val->rolE );
			/*
	...
	public function revisar_cuentas($idmandante,$idusuario,$datos_cuentas){
		$idcuenta = '';
	   ...
	   // echo 'entro';
		...
		for($i = 2; $i <= $rowcount; $i++) {
		  ...
		  /* if (count($usuario) == '1' ){
                echo $usuario->id.'=='.$usuario->nombres;
           	} */	
		...
		/* if (count($cuentas) == '1' ){
                echo $cuentas->id.'=='.$usuario->nombres;
           	}  */	
