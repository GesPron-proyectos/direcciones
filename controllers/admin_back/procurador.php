...
class Procurador extends CI_Controller {
	...
	function __construct() {
	    ...
	    $b=$this->etapas_m->order_by('codigo','ASC')->get_all(); 
		$this->data['etapas'][0]='Seleccionar..';
	 	foreach ($b as $obj) {$this->data['etapas'][$obj->id] = $obj->etapa;}
...
//print_r($where);			
	//    	print_r($_REQUEST);			
			...
			{			
				...
				$order_by ="exorto asc,dias_ desc, cta.id_etapa asc";
			}
	...
	function reporte($tipo = 'pagos',$param = ''){
		...
		foreach ( $cuentas as $key => $val ) {
		    ...
		    $fecha_ultimo_pago = date('d-m-Y',strtotime($val->fecha_ultimo_pago));		
		    }
	...
	function dropdown_ajax_etapa($id_etapa=''){
		...
		$dat = $this->etapas_m->get_many_by(array('activo'=>'S','codigo !='=>''));
	...
	public function fecha_etapa($id_etapa=''){
 	...
 	public function exportar_excel(){
		...
		$sheet = $excel->getActiveSheet ();
		$sheet->SetCellValue ( 'A1', 'MANDANTE' );
		$sheet->SetCellValue ( 'B1', 'JUZGADO' );
		$sheet->SetCellValue ( 'C1', 'JURISDICCION' );
		$sheet->SetCellValue ( 'D1', 'ROL' );
		$sheet->SetCellValue ( 'E1', 'DEUDOR' );
		$sheet->SetCellValue ( 'F1', 'JUZGADO EXHORTO' );
		$sheet->SetCellValue ( 'G1', 'JURISDICCION EXHORTO' );		
		$sheet->SetCellValue ( 'H1', 'ROL EXHORTO' );
		$sheet->SetCellValue ( 'I1', 'ETAPA ANTERIOR' );
		/*
		...
		$sheet->SetCellValue ( 'J1', 'ETAPA ACTUAL' );
		...
		if ($this->data ['nodo']->nombre == 'fullpay') {
		    ...
		    $sheet->SetCellValue ( 'L1', 'FECHA ETAPA ACTUAL' ); 
		...
		foreach ( $cuentas_procurador as $key => $val ) {
			...
			if ($this->data ['nodo']->nombre == 'fullpay') {
				...
				if (count($cuentas_etapas)>0){
					...
					$fecha_etapa = date('d-m-Y',strtotime($cuentas_etapas[0]->fecha_etapa));
					$vv = $cuentas_etapas[0]->etapa.' ('.date('d-m-Y',strtotime($cuentas_etapas[0]->fecha_etapa)).')';
					//echo "www";
					$etapa_actual_ = $cuentas_etapas[0]->etapa.' ('.date('d-m-Y',strtotime($cuentas_etapas[0]->fecha_etapa)).')';
					
				}
				if (count($cuentas_etapas)>1){
					
		     		$vv = $cuentas_etapas[1]->etapa.' ('.date('d-m-Y',strtotime($cuentas_etapas[1]->fecha_etapa)).')';
		     		$observacion_anterior = $cuentas_etapas[1]->observaciones; 
		     		$etapa_anterior_ = $cuentas_etapas[1]->etapa.' ('.date('d-m-Y',strtotime($cuentas_etapas[1]->fecha_etapa)).')';
		        } //else {
				//	$vv = '-';
				//}
				...
				if ($val->fecha_inicio!='' &&  $val->fecha_inicio != '0000-00-00'){
					$fecha_inicio = date('d-m-Y',strtotime($val->fecha_inicio));
				}
		...
		// echo $val->etapa.' '.$val->idce.' '.$fecha_etapa.'<br>';
			...
			$sheet->SetCellValue ( 'C' . $i, $val->distrito );
			$sheet->SetCellValue ( 'D' . $i, $val->rol );
			$sheet->SetCellValue ( 'E' . $i, $val->ap_pat . '  '.$val->nombres );
			$sheet->SetCellValue ( 'F' . $i, $val->tribunalE );
			$sheet->SetCellValue ( 'G' . $i, $val->DistritoE );
			
			$sheet->SetCellValue ( 'H' . $i, $val->rolE );
			
			
			if ($this->data ['nodo']->nombre == 'fullpay') {
				$sheet->SetCellValue ( 'I' . $i, $etapa_anterior_);
			}
			
			if ($this->data ['nodo']->nombre == 'swcobranza') {
				$sheet->SetCellValue ( 'J' . $i, $hh );
			}
			
			$sheet->SetCellValue ( 'K' . $i, $etapa_actual_ );
			...
			if ($this->data ['nodo']->nombre == 'fullpay') {
				...
				$sheet->SetCellValue (  'L' . $i, $val->observaciones );
				$sheet->SetCellValue (  'M' . $i, $fecha_etapa );
