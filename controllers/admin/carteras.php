<?php
class Carteras extends CI_Controller {
	public $data = array();
	public $activo = 'S';
	protected $show_tpl = TRUE;
	public function Carteras() { $this->__construct(); }
...
//###########		
	...
	function index($action='',$id='') {
	    ...
	    //	$this->db->where('padre', 0);
			...
			$query_total = $this->db->get('0_cuentas_roles cta');					 
			...
			//$query =
				...
				a2.apellidos AS prueba
				');
			
			$query2 = $this->db_cae->get('0_administradores a2');
			...
			//$this->db->order_by($order_by);
			$this->db->group_by("cta.id");
			$query =  $this->db->get('0_cuentas_roles cta', $config['per_page'], $this->data['current_pag']);
			...
			if (!$this->show_tpl){  //die('sss');
				$this->data['plantilla'] = 'cuentas/list_roles'; 
	      ...
	      //  $this->output->enable_profiler(TRUE);
			...
			/*	 ->join("2_cuentas_direccion 2cd", "2cd.id_cuenta = cta.id","left")
				...
				$sql_body = 
					...
					" LEFT JOIN `0_cuentas_roles` cta ON cta.id = ce.id_cuenta ".
			...
			else 
			{
				$sql_body = 
					...
					" JOIN `0_cuentas_roles` cta ON cta.id = ce.id_cuenta ".
	...
	function cruce_pjud(){
        ...
        $this->dbaux = $this->load->database("superir", TRUE);

		$result = $this->dbaux->select('cta.*, 
								trib.tribunal, 
								trib.id_tribunal as id_trib_pjud,
								u.rut,
		...
		foreach ($result as $key => $value){
			if($value->letraC){
				$this->dbaux->select('*');
				$this->dbaux->where('id', $value->anio);
				$query = $this->dbaux->get('anio');
				$data = $query->result(); //print_r($data); die;
				if($data)
					$anio = $data[0]->anio;
				else
					$anio = 0;
				$rol = 'C-'.$value->rol.'-'.$anio;
			}
			elseif($value->letraE){
				$this->dbaux->select('*');
				$this->dbaux->where('id', $value->anioE);
				$query = $this->dbaux->get('anio');
				$data = $query->result(); //print_r($data); die;
				if($data)
					$anio = $data[0]->anio;
					...
					$anio = 0;
				$rol = 'E-'.$value->rol.'-'.$anio;
			}
			$data_save = array();
			if($value->activo)
				$data_save['activo'] = $value->activo;
			if($value->posicion)
				$data_save['posicion'] = $value->posicion;
			$data_save['rol'] = $rol;
			if($value->tribunal)
				$data_save['tribunal'] = $value->tribunal;
			if($value->fecha_asignacion)
				$data_save['fecha_ingreso'] = $value->fecha_asignacion;
			//if($value->caratulado)
			//	$data_save['caratulado'] = $value->caratulado;
			// REVISAR CON JORGE EL ABOGADO
			if($value->id_abogado)
				$data_save['id_abogado'] = $value->id_abogado;
			if($value->id_trib_pjud)
				$data_save['id_tribunal'] = $value->id_trib_pjud;
			if($value->fecha_crea)
				$data_save['fecha_crea'] = $value->fecha_crea;
			if($value->user_crea)
				$data_save['user_crea'] = $value->user_crea;
			if($value->ip_crea)
				$data_save['ip_crea'] = $value->ip_crea;
			if($value->rut)
				$data_save['rut'] = $value->rut;
			if($value->fecha_etapa)
				$data_save['fecha_etapa'] = $value->fecha_etapa;
			if($value->etapa)
				$data_save['etapa'] = $value->etapa;
			if($value->estado)
				$data_save['estado'] = $value->estado;
			if($value->nombres)
				$data_save['nombres'] = $value->nombres.' '.$value->apellidos;
			if($value->id_mandante)
				$data_save['mandante'] = $value->id_mandante;

			//print_r($value); die;
			//formatear rol
			
			$id_c = $this->cuentas_roles->insert($data_save, false, true);
			if($id_c){
				...
				$aux = $url;
				$url = $url.'ROL_Causa='.$rol[1].'&TIP_Causa='.$rol[0].'&ERA_Causa='.$rol[2].'&COD_Tribunal='.$value->id_trib_pjud.'&TIP_Informe=1&';
				//echo $url; die;
				$curl = curl_init();
		        ...
		        if($div){
			        ...
			        //echo $aaa; die;
			        //echo utf8_decode($etapa).'--->'.utf8_decode($tramite).'--->'.utf8_decode($descripcion).'--->'.Date('Y-m-d', strtotime($fecha)); 
			        //echo $url_doc; die;
			        $data_save = array();
			        $data_save['id_cuenta'] = $id_c;
			        ...
			        $this->movimiento_cuenta_roles->insert($data_save, false, true);
			    }
		    }
		} //die;
		$this->index();
