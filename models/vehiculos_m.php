<?php
class vehiculos_M extends EMP_Model{
	public function EMP_Model() { $this->__construct(); }

	public function __construct(){
		parent::__construct();
		$this->_table = '2_cuentas_garantia_vehiculo';
		$this->primary_key = 'id';
		$this->alias = 'b';
  ...
  public function eliminar_bienes($id){
	...
	public function get_cuentas_vehiculos($where){


		
		$cols[] = 'b.marca AS marca';
		
		$this->db->select($cols);
		
	    $this->db->from("2_cuentas_bienes b");
	    $this->db->join("0_cuentas c","c.id=b.id_cuenta","left");
        $this->db->where(array('c.activo' => 'S'));
	   //$this->db->where(array('b.id_cuenta'=>$id_cuenta));
